...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Geracao da terceira primaria e suas multiplas
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

t03 = 0.35; % tempo de transito da segunda primaria no offset 0m
v3 = 1750; % velocidade de empilhamento da segunda primaria
coef_ref3=0.3;
tau_v3 = tau_1500 + t03 - t0_fundo_mar;
amp_v3 = coef_ref3*amp_1500;
vel_v3= v3*ones(length(tau_v3),1);
...
% primarias
[dado_p_v3,offsets,tempo] = hyperbolic_events(dt,f,tmax,h,tau_p,vel_p,amp_p,SNR,1);

% dado completo
[dado_v3] = hyperbolic_events(dt,f,tmax,h,tau_v3,vel_v3,amp_v3,SNR,1);

figure(5); imagesc(offsets,tempo,dado_v3);
figure(6); imagesc(offsets,tempo,dado_p_v3);
...
dado_div_v3 = dado_v3.*Div_esf;
dado_p_div_v3 = dado_p_v3.*Div_esf;

figure(7)
wigb(dado_v3,1,h,tempo)
title('Tra�os originais')

figure(8)
wigb(dado_div_v3,1,h,tempo)
title('Tra�os com compensa��o de diverg�ncia esf�rica')
% Dado sem correcao de divergencia esferica
dado_completo = dado + dado_v3;

% Dado com divergencia esferica
dado_completo_div = dado_div + dado_div_v2 + dado_div_v3;
dado_completo_primarias_div = dado_p_div + dado_p_div2 + dado_p_div_v3;
...
save('traco_parametros', 'dt', 'h', 't0_fundo_mar', 't02', 'v2', 't03', 'v3');
