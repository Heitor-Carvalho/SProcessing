...
data_set_name = 'SynData_025';
load('SynData_025_prediction_step.mat')
load([path, 'tracos_in_time.mat']);
load([path, 'tracos_in_radon']);
load([path, 'parameter']);

load_filtered = 1;
load_filtered_file = 'data_set025_filtered.mat'
...
min_offset_idx = 21;
max_offset_idx = 210;
train_matrix_filtered(:, max_offset_idx+1:end) = 0;
train_matrix_filtered_med(:, max_offset_idx+1:end) = 0;
primaries_time = forward_radon_freq(traces_matrix_prim, dt, h(min_offset_idx:max_offset_idx), q, 1, flow, fhigh);
primaries_multiples_time = forward_radon_freq(traces_matrix, dt, h(min_offset_idx:max_offset_idx), q, 1, flow, fhigh);
filtered_traces_time = forward_radon_freq(train_matrix_filtered, dt, h(min_offset_idx:max_offset_idx), q, 1, flow, fhigh);
filtered_traces_time_medfilt = forward_radon_freq(train_matrix_filtered_med, dt, h(min_offset_idx:max_offset_idx), q, 1, flow, fhigh);
