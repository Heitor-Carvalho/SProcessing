...
addpath('../../IA353/RadialBasisNetwork/')
  ...
  for j = 1:length(regularization)
    for k = 1:length(filter_len)
      for l = 1:length(mid_layer_sz)
        ...
        clear nn
        nn = calc_rbf_network(train_set, target, mid_layer_sz(l), regularization(j));
        
        % Calculating extreme learning machines values
        nn_{test_counter} = nn;

        % Neural network prediction
        predicted_trace(:, test_counter) = neural_nete_rbf(train_set, nn);
